...
class CRadioModule : public CProjectConfiguration {
  ...
  private:
    ...
    CHashMap<uint16_t, k_timeout_t> telemetryDownlinkTimes = {{sensorModuleTelemetryPort, K_SECONDS(5)},
    ...
    CUdpListenerTenant powerModuleListenerTenant{"Power Module Listener Tenant", ipAddrStr.c_str(),
                                                 powerModuleTelemetryPort, &powerModuleTelemetryMessagePort};

    CSntpServerTenant sntpServerTenant = *CSntpServerTenant::GetInstance(rtc, CIPv4(ipAddrStr.c_str()));

    CDownlinkSchedulerTenant downlinkSchedulerTenant{loraDownlinkMessagePort, telemetryMessagePortMap,
    ...
    CFrequencyChangeHandler frequencyChangeHandler{lora, loraDownlinkMessagePort,
