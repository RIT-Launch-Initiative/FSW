...
K_THREAD_DEFINE(storage_thread, 2048, flash_storage_thread_entry, NULL, NULL, NULL,
...
static void flash_storage_thread_entry(){
    ...
    while(1){
        ...
        while(1){
            ...
            if(k_msgq_get(&adc_data_queue, &sample, K_MSEC(50)) == 0){
                int ret = flash_write(flash_dev, current_write_addr, &sample, sizeof(sample));
                if(ret < 0){
                    LOG_ERR("Flash write failed (%d)", ret);
                } else {
                    current_write_addr += sizeof(sample);
                }
            }
        }
    }
...
int flash_dump_all(const struct shell *shell){
    ...
    for(test_index = 0; test_index < current_test_number; test_index++){
        off_t block_addr = get_test_block_addr(test_index);
        if (flash_block_is_empty(block_addr)) continue;
