...
static void bme280_compensate_temp(struct bme280_data *data, int32_t adc_temp) {
    ...
    var2 = (((((adc_temp >> 4) - ((int32_t) data->dig_t1)) * ((adc_temp >> 4) - ((int32_t) data->dig_t1))) >> 12) *
            ((int32_t) data->dig_t3)) >>
           14;
...
static void bme280_compensate_humidity(struct bme280_data *data, int32_t adc_humidity) {
    ...
    h = ((((adc_humidity << 14) - (((int32_t) data->dig_h4) << 20) - (((int32_t) data->dig_h5) * h)) +
          ((int32_t) 16384)) >>
         15) *
        (((((((h * ((int32_t) data->dig_h6)) >> 10) * (((h * ((int32_t) data->dig_h3)) >> 11) + ((int32_t) 32768))) >>
            10) +
           ((int32_t) 2097152)) *
              ((int32_t) data->dig_h2) +
          8192) >>
...
static int bme280_channel_get(const struct device *dev, enum sensor_channel chan, struct sensor_value *val) {
    struct bme280_data *data = dev->data;
    ...
    switch (chan) {
		 ...
		 */
            ...
            val->val2 = (data->comp_press >> 8) % 1000 * 1000U + (((data->comp_press & 0xff) * 1000U) >> 8);
            break;
...
#define BME280_DEFINE(inst)                                                                                            \
    ...
    static const struct bme280_config bme280_config_##inst =                                                           \
        COND_CODE_1(DT_INST_ON_BUS(inst, spi), (BME280_CONFIG_SPI(inst)), (BME280_CONFIG_I2C(inst)));                  \
                                                                                                                       \
    PM_DEVICE_DT_INST_DEFINE(inst, bme280_pm_action);                                                                  \
                                                                                                                       \
    SENSOR_DEVICE_DT_INST_DEFINE(inst, bme280_chip_init, PM_DEVICE_DT_INST_GET(inst), &bme280_data_##inst,             \
                                 &bme280_config_##inst, POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY, &bme280_api_funcs);
