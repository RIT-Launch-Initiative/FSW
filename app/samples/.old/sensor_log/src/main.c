...
SensorLogger detect_buffer {
    ...
    L_FS_CREATE_FILE flight_log{flight_samples_path, sizeof(struct txyz), N_ONCE_SAMPLES, SLOG_ONCE};
    ...
    void print_mag_val(struct txyz * val) {
        LOG_PRINTK("T: %d ct\tX: %5f ga\tY: %5f ga\tZ: %5f ga\n", val->time, sensor_value_to_double(val->xyz),
                   sensor_value_to_double(val->xyz + 1), sensor_value_to_double(val->xyz + 2));
    }
    ...
    int32_t log_routine(void) {
        ...
        int32_t end = now + CONFIG_SENSOR_LOG_DURATION * CONFIG_SYS_CLOCK_TICKS_PER_SEC;
        struct txyz sample;
