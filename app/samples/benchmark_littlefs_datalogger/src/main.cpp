...
template <typename T>
void benchmarkDataloggerMode(const char *test_name, const char *filename, LogMode mode, size_t max_packets = 0) {
    LOG_INF("=== %s ===", test_name);
    ...
    double avgWriteMs = (double) totalMs / NUM_ITERATIONS;
    ...
    LOG_INF("Average write time: %.3f ms", avgWriteMs);
...
void benchmarkFileOperations() {
    ...
    const char *testFiles[] = {"/lfs/test1.bin", "/lfs/test2.bin", "/lfs/test3.bin", "/lfs/test4.bin",
                               "/lfs/test5.bin"};
    ...
    uint32_t create_startMs = k_uptime_get_32();
    ...
    uint32_t createMs = create_endMs - create_startMs;
...
int main() {
    ...
    benchmarkDataloggerMode<SmallPacket>("CDataLogger Small Packet (Growing Mode)", "/lfs/small_growing.bin",
                                         LogMode::Growing);

    benchmarkDataloggerMode<MediumPacket>("CDataLogger Medium Packet (Growing Mode)", "/lfs/medium_growing.bin",
                                          LogMode::Growing);

    benchmarkDataloggerMode<LargePacket>("CDataLogger Large Packet (Growing Mode)", "/lfs/large_growing.bin",
                                         LogMode::Growing);

    // Benchmark different modes
    benchmarkDataloggerMode<MediumPacket>("CDataLogger Medium Packet (Circular Mode)", "/lfs/medium_circular.bin",
                                          LogMode::Circular, 500);

    benchmarkDataloggerMode<MediumPacket>("CDataLogger Medium Packet (FixedSize Mode)", "/lfs/medium_fixed.bin",
                                          LogMode::FixedSize, 500);
