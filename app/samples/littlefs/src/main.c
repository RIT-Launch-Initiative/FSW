...
void print_stat(char* fname) {
    ...
    if (ret < 0) {
        ...
        LOG_INF("%s is a %s of size %d", stat_dst.name, stat_dst.type == FS_DIR_ENTRY_FILE ? "file" : "directory",
                stat_dst.size);
    }
}
...
void print_statvfs(char* fname) {
    ...
    if (ret < 0) {
        ...
        LOG_INF("%s is on a volume with \r\n\t%lu blocks (%lu free) of %lu bytes each", fname, fs_stat_dst.f_blocks,
                fs_stat_dst.f_bfree, fs_stat_dst.f_frsize);
        LOG_PRINTK("\t");
...
int32_t clear_flash_area(uint8_t fa_id) {
    ...
    LOG_INF("Area %u at 0x%x on %s for %u bytes\n", fa_id, (uint32_t) area_p->fa_off, area_p->fa_dev->name,
            area_p->fa_size);
