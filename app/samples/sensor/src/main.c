...
int32_t read_mag() {
    ...
    LOG_PRINTK("KM: %6d ms\tKT: %10d tck\tCK: %10u cyc\tX: %4fga\tY: %4fga\tZ: %4fga\n", k_uptime_get_32(),
               (int32_t) k_uptime_ticks(), k_cycle_get_32(), sensor_value_to_double(val),
...
int main(void) {
    ...
    k_thread_create(&read_mag_thread_data, read_mag_stack, K_THREAD_STACK_SIZEOF(read_mag_stack), &read_mag_thread,
