...
ZTEST(golay, test_encode_decode_no_errors) {
    ...
    zassert_equal(decoded, data, "Decoded data should match original");
}
...
ZTEST(golay, test_encode_multiple_values) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        zassert_equal(decoded, data, "Decoded should match original for 0x%03x", data);
    }
}
...
ZTEST(golay, test_single_bit_error_correction) {
    ...
    zassert_equal(decoded, data, "Single bit error should be correctable");
}
...
ZTEST(golay, test_two_bit_error_correction) {
    ...
    zassert_equal(decoded, data, "Two bit errors should be correctable");
}
...
ZTEST(golay, test_three_bit_error_correction) {
    ...
    zassert_equal(decoded, data, "Three bit errors should be correctable");
}
...
ZTEST(golay, test_error_counting) {
    ...
    zassert_equal(error_count, 2, "Error count should be 2");
}
...
ZTEST(golay, test_zero_data) {
    ...
    zassert_equal(decoded, data, "Zero should encode and decode correctly");
}
...
ZTEST(golay, test_all_ones_data) {
    ...
    zassert_equal(decoded, data, "All ones should encode and decode correctly");
}
