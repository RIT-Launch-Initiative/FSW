...
static void ina_task(void *, void *, void *) {
  ...
  while (true) {
    ...
    l_get_sensor_data(sensors[0], 3, ina_channels,
                      (struct sensor_value **)&data_battery);
    l_get_sensor_data(sensors[1], 3, ina_channels,
                      (struct sensor_value **)&data_3v3);
    l_get_sensor_data(sensors[2], 3, ina_channels,
                      (struct sensor_value **)&data_5v0);
  }
}
...
//     };
//
//     static const struct adc_channel_cfg vin_volt_sens_channel =
//     ADC_CHANNEL_CFG_DT(adc1);
//     // if (!adc_is_ready_dt()) {
...
void init_telem_tasks() {
  ...
  for (int i = 0; i < 3; i++) {
    k_thread_create(&threads[i], &stacks[i][0], STACK_SIZE, ina_task,
...
void convert_and_send() {
  ...
  packet.current_battery =
      sensor_value_to_float(&power_module_data.ina_battery.current);
  packet.voltage_battery =
      sensor_value_to_float(&power_module_data.ina_battery.voltage);
  packet.power_battery =
      sensor_value_to_float(&power_module_data.ina_battery.power);

  packet.current_3v3 =
      sensor_value_to_float(&power_module_data.ina_3v3.current);
  packet.voltage_3v3 =
      sensor_value_to_float(&power_module_data.ina_3v3.voltage);
  packet.power_3v3 = sensor_value_to_float(&power_module_data.ina_3v3.power);

  packet.current_5v0 =
      sensor_value_to_float(&power_module_data.ina_5v0.current);
  packet.voltage_5v0 =
      sensor_value_to_float(&power_module_data.ina_5v0.voltage);
  packet.power_5v0 = sensor_value_to_float(&power_module_data.ina_5v0.power);
  packet.vin_voltage_sense = flip_flop ? 0xDEAD : 0xBEEF;
  flip_flop ^= 0b1;

  l_send_udp_broadcast((const uint8_t *)&packet, sizeof(power_module_packet_t),
                       9000);
}
