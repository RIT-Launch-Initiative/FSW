...
enum adxl375_axis { ADXL375_X_AXIS, ADXL375_Y_AXIS, ADXL375_Z_AXIS };

enum adxl375_op_mode { ADXL375_STANDBY = 0x04, ADXL375_MEASUREMENT = 0x08, ADXL375_AUTOSLEEP = 0x24 };
...
enum adxl375_fifo_mode { ADXL375_FIFO_BYPASS, ADXL375_FIFO_STREAM, ADXL375_FIFO_TRIGGER };
...
struct adxl375_transfer_function {
    int (*read_reg_multiple)(const struct device *dev, uint8_t reg_addr, uint8_t *value, uint16_t len);
    ...
    int (*write_reg_mask)(const struct device *dev, uint8_t reg_addr, uint32_t mask, uint8_t value);
};
