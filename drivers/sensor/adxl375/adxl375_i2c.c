...
static int adxl375_bus_access(const struct device *dev, uint8_t reg, void *data, size_t length) {
    ...
    if (reg & ADXL375_READ) {
        return i2c_burst_read_dt(&config->i2c, ADXL375_TO_I2C_REG(reg), (uint8_t *) data, length);
    } else {
        if (length != 1) {
            return -EINVAL;
        }

        return i2c_reg_write_byte_dt(&config->i2c, ADXL375_TO_I2C_REG(reg), *(uint8_t *) data);
    }
}
...
static int adxl375_i2c_reg_read_multiple(const struct device *dev, uint8_t reg_addr, uint8_t *reg_data,
                                         uint16_t count) {
...
int adxl375_i2c_reg_write_mask(const struct device *dev, uint8_t reg_addr, uint32_t mask, uint8_t data) {
    int ret;
    uint8_t tmp;
