...
static int adxl375_spi_reg_read(const struct device *dev, uint8_t reg_addr,
			    uint8_t *reg_data)
{
	return adxl375_bus_access(dev, ADXL375_REG_READ(reg_addr), reg_data, 1);
}

static int adxl375_spi_reg_read_multiple(const struct device *dev,
					 uint8_t reg_addr,
					 uint8_t *reg_data,
					 uint16_t count)
{
...
static int adxl375_spi_reg_write(const struct device *dev,
				 uint8_t reg_addr,
				 uint8_t reg_data)
{
...
int adxl375_spi_reg_write_mask(const struct device *dev,
			       uint8_t reg_addr,
			       uint32_t mask,
			       uint8_t data)
{
...
static const struct adxl375_transfer_function adxl372_spi_transfer_fn = {
