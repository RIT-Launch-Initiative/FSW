...
static void ms56xx_compensate(struct ms56xx_data *data, const struct ms56xx_chip_calc_coefficients *const coefficients,
    ...
    if (data->temperature < 2000) {
        Ti = (dT * dT) / (1ll << 31);
        OFFi = (coefficients->low_temp_off2_factor * temp_sq) / (1ll << coefficients->low_temp_off2_div_exp);
        SENSi = (coefficients->low_temp_sens2_factor * temp_sq) / (1ll << coefficients->low_temp_sens2_div_exp);
        if (data->temperature < -1500) {
            temp_sq = (int64_t) (data->temperature + 1500) * (int64_t) (data->temperature + 1500);
            OFFi += coefficients->very_low_temp_off2_addend * temp_sq;
            SENSi += (coefficients->very_low_temp_sens2_addend * temp_sq) / coefficients->very_low_temp_sens2_div;
        }
    } else {
        SENSi = 0;
        OFFi = 0;
...
static int ms56xx_sample_fetch(const struct device *dev, enum sensor_channel channel) {
    ...
    err = ms56xx_get_measurement(config, &adc_pressure, data->pressure_conv_cmd, data->pressure_conv_delay);
    if (err < 0) {
        return err;
    }
...
static int ms56xx_channel_get(const struct device *dev, enum sensor_channel chan, struct sensor_value *val) {
...
static int ms56xx_attr_set(const struct device *dev, enum sensor_channel chan, enum sensor_attribute attr,
...
#define MS56XX_CONFIG_07_COEFFICIENTS                                                                                  \
    (const struct ms56xx_chip_calc_coefficients) {                                                                     \
        ...
        .sens_c1_factor_exp = MS5607_SENS_C1_FACTOR_EXP, .sens_c3_dt_div_exp = MS5607_SENS_C3_DT_DIV,                  \
        .low_temp_off2_factor = MS5607_LOW_TEMP_OFF2_FACTOR, .low_temp_off2_div_exp = MS5607_LOW_TEMP_OFF2_DIV_EXP,    \
...
#define MS56XX_CONFIG_11_COEFFICIENTS                                                                                  \
    (const struct ms56xx_chip_calc_coefficients) {                                                                     \
        ...
        .sens_c1_factor_exp = MS5611_SENS_C1_FACTOR_EXP, .sens_c3_dt_div_exp = MS5611_SENS_C3_DT_DIV,                  \
        .low_temp_off2_factor = MS5611_LOW_TEMP_OFF2_FACTOR, .low_temp_off2_div_exp = MS5611_LOW_TEMP_OFF2_DIV_EXP,    \
...
DT_FOREACH_STATUS_OKAY(DT_DRV_COMPAT_MS5607, MS5607_DEFINE)
DT_FOREACH_STATUS_OKAY(DT_DRV_COMPAT_MS5611, MS5611_DEFINE)
