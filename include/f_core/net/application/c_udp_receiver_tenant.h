...
class CUdpReceiveTenant : public CRunnableTenant {
  public:
     ...
     */
    CUdpReceiveTenant(const char *name, const char *ipAddr, const int srcPort, const int dstPort,
                      CMessagePort<T> &messagePort)
        : CRunnableTenant(name), udp(CIPv4(ipAddr), srcPort, dstPort), messagesReceived(&messagePort) {
        udp.SetRxTimeout(0);
    }
     ...
     * @param messagePort Message port to put messages received over UDP
     */
    CUdpReceiveTenant(const CUdpSocket &udp, CMessagePort<T> &messagePort)
        : CRunnableTenant(name), udp(udp), messagesReceived(&messagePort) {
        udp.SetRxTimeout(0);
    }
     ...
     * See parent docs
     */
    void Run() override { ReceiveMessageAsynchronous(); }
