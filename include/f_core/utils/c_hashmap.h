    ...
    bool Insert(const KeyType& key, const ValueType& value) {
        if (!isMainThreadCurrent()) {
            if (!map.contains(key) && size > maxSizeReachedAtStartup) {
                printk(
                    "Attempted to insert more than the maximum size of the hashmap post-startup\n"); // LOG doesn't work well in templates
// Only fatal in debug mode. Unlikely to occur in an actual flight, and in the off-chance it does we shouldn't fatal the entire system over it
    ...
    bool Emplace(const KeyType& key, ValueType&& value) {
        if (!isMainThreadCurrent()) {
            if (!map.contains(key) && size > maxSizeReachedAtStartup) {
                printk(
                    "Attempted to emplace more than the maximum size of the hashmap post-startup\n"); // LOG doesn't work well in templates
#ifdef CONFIG_DEBUG
                k_oops();
#endif
                return false;
    ...
    typename std::unordered_map<KeyType, ValueType>::iterator begin() { return map.begin(); }

    typename std::unordered_map<KeyType, ValueType>::iterator end() { return map.end(); }
  ...
  private:
    ...
    bool isMainThreadCurrent() { return strncmp(k_thread_name_get(k_current_get()), "main", 4) == 0; }
};
