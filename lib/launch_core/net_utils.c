...
int l_create_ip_str_default_net_id(char *ip_str, int c, int d) {
  return l_create_ip_str(ip_str, BACKPLANE_NETWORK_ID[0],
                         BACKPLANE_NETWORK_ID[1], c, d);
}
...
int l_init_udp_net_stack(const char *ip_addr) {
  ...
  struct net_if_addr *ifaddr =
      net_if_ipv4_addr_add(net_interface, &addr, NET_ADDR_MANUAL, 0);
  if (!ifaddr) {
...
int l_send_udp_broadcast(const uint8_t *data, size_t data_len, uint16_t port) {
  ...
  ret = sendto(sock, data, data_len, 0, (struct sockaddr *)&dst_addr,
               sizeof(dst_addr));
  if (ret < 0) {
...
int l_receive_udp_callback(const struct device *dev, struct net_pkt *packet,
                           int status) {
...
int l_add_port_handler(uint16_t port, l_udp_port_handler_t *handler) {
  return sys_hashmap_insert(&UDP_PORT_HANDLERS, port, POINTER_TO_INT(handler),
                            NULL);
}
