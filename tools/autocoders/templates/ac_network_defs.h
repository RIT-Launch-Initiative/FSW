...
#define CREATE_IP_ADDR(base, octet3, octet4)                                                                           \
    (std::string(base) + "." + std::to_string(octet3) + "." + std::to_string(octet4))
...
_MODULE_IP_ADDR_BASE = "10.{{ module_info.id }}";
...
_BASE_PORT = {{module_info.base_port}};
...
_MODULE_ {
    {
        offset_name.upper() | replace("_", "_")
    }
}
_PORT = {{module_info.base_port + offset_info.port_number}};
{
    % endfor %
}
{
